defmodule Olivetree.Accounts do
  ...
  alias Olivetree.Mailer.Email
  alias Olivetree.Accounts.{UsernameGenerator, UserPermissions, Invitations}
  alias Olivetree.Actions.ActionCreator
  ...
  @fetch_default_picture Application.get_env(:olivetree, :fetch_default_user_picture, true)
  ...
  def send_welcome(user) do
    Olivetree.Mailer.deliver_later(Olivetree.Mailer.Email.welcome(user))
  end
  ...
  def fetch_picture(user, picture_url) do
    ...
    if Application.get_env(:olivetree, :env) != :test do
  ...
  def reset_password!(email, source_ip_address) when is_binary(source_ip_address) do
    ...
    |> Olivetree.Mailer.deliver_later()
  end
  ...
  def send_newsletter(subject, html_body, locale_filter \\ nil) do
    ...
    |> Repo.all()
    |> Enum.map(&Olivetree.Mailer.Email.newsletter(&1, subject, html_body))
    |> Enum.map(&Olivetree.Mailer.deliver_later/1)
    |> Enum.count()
  end
